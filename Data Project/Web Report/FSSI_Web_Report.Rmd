---
title: "Social Service Charities in Victoria"
output:
  html_document:
    fig_height: 7
    fig_width: 8
...
# Reading in 2018 data and cleaning it to VCOSS standards

ACNC_18 <- clean_names(read_excel("ACNC Data/ACNC_ais18.xlsx"))

VCOSS_ACNC_18 <- filter(ACNC_18,
                        str_detect(state,
                                   regex("vic|victoria",
                                         ignore_case = TRUE)))

VCOSS_ACNC_18$state <- "Vic"

colnames(VCOSS_ACNC_18)[which(str_detect(colnames(VCOSS_ACNC_18), "government"))] <- "government_grants"

vcoss_colnames <- colnames(VCOSS_ACNC_16)

vcoss_colnames <- str_remove_all(vcoss_colnames,
                                 pattern = "_2016")

vcoss_colnames <- vcoss_colnames[which(!str_detect(vcoss_colnames, "ABN_Validation"))]

VCOSS_ACNC_18 <- select(VCOSS_ACNC_18,
                        vcoss_colnames)

for(col_name in colnames(VCOSS_ACNC_18)) {
  
  if(c("n", "y") %in% unique(VCOSS_ACNC_18[[col_name]]) == c(TRUE, TRUE)) {
    
    VCOSS_ACNC_18[[col_name]] <- recode(VCOSS_ACNC_18[[col_name]],
                                        "n" = FALSE,
                                        "y" = TRUE)
...
## Cleaning main_activity

VCOSS_ACNC_18 <- filter(VCOSS_ACNC_18,
                        !str_detect(main_activity,
                                   regex("religious", ignore_case = TRUE)),
                        !is.na(main_activity))

VCOSS_ACNC_18$main_activity <- str_cleaner(VCOSS_ACNC_18$main_activity)

VCOSS_ACNC_18$main_activity <- str_replace_all(VCOSS_ACNC_18$main_activity,
                                               pattern = regex("emergency and relief",
                                                               ignore_case = TRUE),
                                               replacement = "Emergency Relief")

VCOSS_ACNC_18 <- filter(VCOSS_ACNC_18,
                        main_activity %in% VCOSS_ACNC_16$main_activity_2016)
...
VCOSS_ACNC_18 <- mutate(VCOSS_ACNC_18,
                        ABN_Validation = ABN_Validator(abn))

Invalid_ABNs_18 <- filter(VCOSS_ACNC_18,
                          ABN_Validation == "Invalid ABN")
...
VCOSS_ACNC_18 <- VCOSS_ACNC_18[which(!(VCOSS_ACNC_18$abn %in% Invalid_ABNs_18$abn)), ]

```

# Foreword

The social service sector in Victoria is a multi billion dollar sector providing a range of services with the common focus of reducing poverty and disadvantage experienced by Victorians. The sector employs thousands of Victorians, making a substantial contribution to the state economy. There are a large number of charities and not-for-profit organisations that operate within the social service sector in Victoria, often deriving funding from very different sources than that of their privately-run counterparts.  

As the social service sector in Victoria is so large and complex, this report dives into the data to better understand its composition and characteristics. This report and its data visualisations make use of data provided by the [Australian Charities and Not-for-profits Commission (ACNC)](https://www.acnc.gov.au) to examine the impact of charities and not-for-profit organisations in the social service sector in Victoria. This report also looks at changes in the nature of the charities and not-for-profits in the social service sector between 2016 and 2017.
...
### Preview of Datasets {.tabset .tabset-fade}

Each of the below datasets are extracts of the ACNC datasets for Victorian charities.
To view the complete cleaned datasets for all years, please see **Appendix 1c**
...
VCOSS_ACNC_18_prettycolnames <- str_cleaner(str_replace_all(colnames(VCOSS_ACNC_18),
                                                            pattern = "_", replacement = " "))

datatable(head(arrange(VCOSS_ACNC_18, desc(total_revenue)), 30),
          rownames = FALSE, colnames = VCOSS_ACNC_18_prettycolnames,
          class = "display compact",
          options = list(bFilter = FALSE,
                         lengthChange = FALSE,
                         scrollY = "400px",
...
Count_MainAct_18 <- summarise(group_by(VCOSS_ACNC_18,
                                       main_activity),
                              Count = n())

SocServ18 <- Count_MainAct_18$Count[which(str_detect(Count_MainAct_18$main_activity, "Social Services"))]

TotalCount18 <- sum(Count_MainAct_18$Count)

Count_MainAct_18 <- arrange(Count_MainAct_18, desc(Count))

```

Charities operating in the social service sector comprised more than one out of every five charities operating in Victoria in 2016 and 2017. In 2016 this was `r comma(SocServ16, 1)` charities, which decreased slightly in 2017 to `r comma(SocServ17, 1)` (`r percent(SocServ16 / TotalCount16, 0.1)` and `r percent(SocServ17 / TotalCount17, 0.1)` of total charities, respectively). In 2018 the number of social service charities decreased again to `r comma(SocServ18, 1)` (`r percent(SocServ18 / TotalCount18, 0.1)`), which was in line with a decrease across all charities (`r comma(TotalCount18, 1)` total charities).   

In 2016, the largest number of charities were working in the `r Count_MainAct_16$main_activity_2016[1]` sector with `r Count_MainAct_16$main_activity_2016[2]` recording the second-most number of charities at `r comma(Count_MainAct_16$Count[2], 1)`. These figures changed in 2017, with `r Count_MainAct_17$main_activity[1]` recording the greatest number of charities at `r comma(Count_MainAct_17$Count[1], 1)` while `r Count_MainAct_17$main_activity[2]` decreased slightly at `r comma(Count_MainAct_17$Count[2], 1)` charities.
...
### Social Service and all other Sectors
...
Counts_2018 <- mutate(VCOSS_ACNC_18,
                      Soc_Serv_Bool = if_else(str_detect(main_activity, 
                                                         regex("social services",
                                                               ignore_case = TRUE)),
                                              true = "Social Services",
...
Counts_2018 <- summarise(group_by(Counts_2018,
                                  Soc_Serv_Bool),
                         Count = n())

Counts_2018$Year <- 2018

Counts_678 <- rbind(Counts_2016, Counts_2017, Counts_2018)

Counts_678$Year <- as.ordered(Counts_678$Year)

gg2_Counts_678 <- ggplot(Counts_678,
       aes(x = Year,
           y = Count,
           col = Soc_Serv_Bool,
           group = Soc_Serv_Bool,
...
ggplotly(gg2_Counts_678,
         tooltip = "text") %>% 
  layout(font = list(family = FSSIfont,
                     size = 12),
         title = list(text = "Number of Victorian Charities in Social Service and Other Sectors",
                      xref = "canvas",
                      x = 0.1),
...
Vic_Char_Count18 <- summarise(group_by(VCOSS_ACNC_18,
                                       main_activity = as.factor(main_activity)),
                              "Count" = n())

Vic_Char_Count18$Year <- 2018

Vic_Char_Count_678 <- rbind(Vic_Char_Count16, Vic_Char_Count17, Vic_Char_Count18)

Vic_Char_Count_678$Year <- as.ordered(Vic_Char_Count_678$Year)

Vic_Char_Count_678 <- replace_na(Vic_Char_Count_678,
                                  replace = list(main_activity = "Not Specified"))


## bar chart

gg_Vic_Char_Count_678 <- ggplot(Vic_Char_Count_678,
                                 aes(x = Year, y = Count,
  ...
  scale_colour_manual("",
                      values = c(rep(FSSIQualSingle(2), length(unique(Vic_Char_Count_678$main_activity))-1),
                                 FSSIGreenSingle)) +
  theme_minimal()

ggplotly(gg_Vic_Char_Count_678,
         tooltip = "text") %>% 
  layout(font = list(family = FSSIfont,
                     size = 12),
         title = list(text = "Number of Victorian Charities per Main Activity, 2016 to 2018",
                      standoff = 10,
                      xref = "canvas",
                      x = 0.1),
...
## Change in Social Service Charity Sizes
...
VIC_SocServ18 <- filter(VCOSS_ACNC_18,
                        str_detect(main_activity, regex("social services", ignore_case = TRUE)))

VIC_SocServ18$Year <- 2018

VIC_SocServ18 <- select(VIC_SocServ18,
                        main_activity, charity_size, Year)

VIC_SocServ_678 <- rbind(VIC_SocServ16, VIC_SocServ17, VIC_SocServ18)

VIC_SocServ_678$Year <- as.ordered(VIC_SocServ_678$Year)

Sum_VIC_SocServ_678 <- summarise(group_by(VIC_SocServ_678,
                                           Year, charity_size),
                                  n_Charities = n())

gg_Sum_VIC_SocServ_678 <- ggplot(Sum_VIC_SocServ_678,
  ...
  scale_y_continuous("Number of Charities",
                     labels = comma,
                     limits = c(0, max(Sum_VIC_SocServ_678$n_Charities)*1.1),
...
ggplotly(gg_Sum_VIC_SocServ_678,
         tooltip = "text") %>% 
  layout(font = list(family = FSSIfont,
                     size = 12),
         title = list(text = "Number of Social Service Charities, 2016 to 2018",
                      standoff = 10,
                      xref = "canvas",
                      x = 0.1),
...
Social services charities had the largest number of total employees as well as full-time and casual type employees in 2016 but in 2017, the largest number of staff were in aged care charities. The number of staff in social service charities accounted for 23.45% of total number of charity staff in 2016 and increased slightly to 23.53% in 2017. Conversely, the total number of staff in organizations performing aged care activities increased from 20.2% to 24.6%. Other health service delivery charities saw the second largest number of employees and aged care activities had the third largest number of employees, which were 32,574 and 30,418 respectively in 2016. Things changed in 2017 while social service sectors and other education sectors were in the second and third place regarding to numbers of staffs. Income support and maintenance and civic and advocacy activities are seen having the smallest proportions of staffs.
...
SocServ_Staff_MainAct18 <- summarise(group_by(filter(VCOSS_ACNC_18,
                                                     str_detect(main_activity,
                                                                regex("social services", ignore_case = TRUE))),
                                              main_activity),
                                     full_time = sum(staff_full_time, na.rm = TRUE),
                                     part_time = sum(staff_part_time, na.rm = TRUE),
                                     casual = sum(staff_casual, na.rm = TRUE))

SocServ_Staff_MainAct18_long <- pivot_longer(SocServ_Staff_MainAct18,
                                             cols = c(full_time,
                                                      part_time,
                                                      casual),
                                             names_to = "employment_type",
...
SocServ_Staff_MainAct18_long$Year <- 2018

SocServ_Staff_MainAct_678 <- rbind(SocServ_Staff_MainAct16_long,
                                   SocServ_Staff_MainAct17_long,
                                   SocServ_Staff_MainAct18_long)

SocServ_Staff_MainAct_678$Year <- as.ordered(SocServ_Staff_MainAct_678$Year)

gg_SocServ_Staff_MainAct_678 <- ggplot(SocServ_Staff_MainAct_678,
                                        aes(x = Year,
                                            y = employee_count,
...
ggplotly(gg_SocServ_Staff_MainAct_678,
         tooltip = "text") %>% 
  layout(title = list(text = "Changes in Staff Counts by Work Loading in Social Service, 2016 to 2018",
                      standoff = 10,
                      xref = "canvas",
                      x = 0.1),
...
# 2018
## staff_casual and staff_volunteers are character variables

VCOSS_ACNC_18$staff_casual <- as.numeric(VCOSS_ACNC_18$staff_casual)

VCOSS_ACNC_18$staff_volunteers <- as.numeric(VCOSS_ACNC_18$staff_volunteers)

SocServ_Staff_CharSize_18 <- summarise(group_by(filter(VCOSS_ACNC_18,
                                                     str_detect(main_activity,
                                                                regex("social services", ignore_case = TRUE))),
                                              charity_size),
                                     staff_part_time = sum(staff_part_time, na.rm = TRUE),
                                     staff_full_time = sum(staff_full_time, na.rm = TRUE),
                                     staff_casual = sum(as.numeric(staff_casual), na.rm = TRUE))


SocServ_Staff_CharSize_18 <- pivot_longer(SocServ_Staff_CharSize_18,
                                        cols = c(staff_part_time,
                                                 staff_full_time,
                                                 staff_casual),
                                        names_to = "employment_type",
...
SocServ_Staff_CharSize_18$Year <- 2018

# binding

SocServ_Staff_CharSize_1617 <- rbind(SocServ_Staff_CharSize_16,
                                     SocServ_Staff_CharSize_17,
                                     SocServ_Staff_CharSize_18)
...
subplot(pl_SocServ_Staff_CharSize_1617, pl_SocServ_Staff_CharSize_1617_prop,
  ...
  layout(title = list(text = "Social Service Staff Employment Types per Charity Size",
                      xref = "canvas",
                      x = 0.1),
...
Volunteers_MainAct_18 <- summarise(group_by(VCOSS_ACNC_18,
                                            main_activity),
                                   volunteers = sum(as.numeric(staff_volunteers)))

Volunteers_MainAct_18$Year <- 2018

# binding

Volunteers_MainAct_678 <- rbind(Volunteers_MainAct_16,
                                 Volunteers_MainAct_17,
                                 Volunteers_MainAct_18)

Volunteers_MainAct_678$Year <- as.ordered(Volunteers_MainAct_678$Year)

gg_Volunteers_MainAct_678 <- ggplot(Volunteers_MainAct_678,
                                     aes(x = Year,
                                         y = volunteers,
  ...
  scale_color_manual("Main Activity",
                     values = c(rep(FSSIQualSingle(2),
                                    length(unique(Volunteers_MainAct_678$main_activity))-1),
                                FSSIGreenSingle)) +
  theme_minimal()

ggplotly(gg_Volunteers_MainAct_678,
         tooltip = "text") %>% 
...
Volunteers_PaidStaff_2018 <- summarise(group_by(VCOSS_ACNC_18,
                                                main_activity, charity_size),
...
Volunteers_PaidStaff_2018$Year <- 2018

Volunteers_PaidStaff_678 <- rbind(Volunteers_PaidStaff_2016,
                                  Volunteers_PaidStaff_2017,
                                  Volunteers_PaidStaff_2018)

```

```{r echo=FALSE, message=FALSE, warning=FALSE}

gg_SocServ_Volunteers_PaidStaff_678 <- ggplot(filter(Volunteers_PaidStaff_678,
                                                      str_detect(main_activity,
                                                                 regex("social services",
                                                                       ignore_case = TRUE)))) +
  geom_abline(slope = 1,
...
ggplotly(gg_SocServ_Volunteers_PaidStaff_678,
  ...
  layout(legend = list(title = list(text = "Charity size",
                       ...
                       itemclick = "toggleothers",
                       itemdoubleclick = "toggle"),
         title = list(text = "Volunteers and Paid Staff Working for Social Service, 2016 to 2018",
                      xref = "canvas",
                      x = 0.1),
         margin = list(t = 50,
                       b = 10,
...
plot_ly(data = filter(Volunteers_PaidStaff_678_long,
        ...
        marker = list(colors = c("rgb(84, 144, 65))",
                      ...
                      line = list(color = "#cecece",
                                  width = 0.85))) %>% 
  layout(title = list(text = "Volunteer Workforce Share for Social Service, 2016 to 2018",
                      standoff = 10,
                      xref = "canvas",
                      x = 0.1),
         font = list(family = FSSIfont,
...
ggplotly(gg_packing_678,
  ...
  layout(legend = list(title = list(text = "Main Activity",
                       ...
                       itemclick = "toggleothers",
                       itemdoubleclick = "toggle"),
         title = list(text = "Total Revenue for all Victorian Charities, 2016 to 2018",
                      xref = "canvas",
                      x = 0.1),
...
Revenue18 <- summarise(group_by(VCOSS_ACNC_18,
                                main_activity = main_activity),
                       government_grants = sum(government_grants,
                                               na.rm = TRUE),
                       donations_and_bequests = sum(donations_and_bequests,
                                                    na.rm = TRUE),
                       all_other_revenue = sum(all_other_revenue,
                                               na.rm = TRUE),
                       total_revenue = sum(total_revenue,
                                           na.rm = TRUE))
Revenue18$Year <- 2018

Revenue678 <- rbind(Revenue16, Revenue17, Revenue18)

## All other sectors

Other_Revenue678 <- filter(Revenue678,
                            !str_detect(main_activity, regex("social services",
                                                             ignore_case = TRUE)))

Other_Revenue678_long <- pivot_longer(Other_Revenue678,
                                       -c(main_activity, Year, total_revenue),
                                       names_to = "revenue_source",
                                       values_to = "revenue_amount")

Total_Other_Revenue678_long <- summarise(group_by(Other_Revenue678_long,
                                                   revenue_source, Year),
                                          revenue_amount = sum(revenue_amount))

Total_Other_Revenue678_long$main_activity <- "Other Sectors"

# plot

gg_Other_Revenue678 <- ggplot(ungroup(Total_Other_Revenue678_long),
...
pl_Other_Revenue678 <- ggplotly(gg_Other_Revenue678,
                                 tooltip = "text",
...
SocServ_Revenue678 <- filter(Revenue678,
                              str_detect(main_activity, regex("social services",
                                                               ignore_case = TRUE)))


SocServ_Revenue678_long <- pivot_longer(SocServ_Revenue678,
                                       -c(main_activity, Year, total_revenue),
                                       names_to = "revenue_source",
                                       values_to = "revenue_amount")

SocServ_Revenue678_long <- mutate(group_by(SocServ_Revenue678_long,
                                          main_activity, Year),
...
gg_SocServ_Revenue678 <- ggplot(SocServ_Revenue678_long,
                                 aes(x = as.ordered(Year),
                                     y = revenue_amount,
                                     colour = str_clean_wrap(revenue_source, 18),
                                     group = revenue_source,
...
pl_SocServ_Revenue678 <- ggplotly(gg_SocServ_Revenue678,
                                   tooltip = "text",
                                   legendgroup = "legendgroup") %>% 
  layout(font = list(family = FSSIfont,
                     size = 12))
...
subplot(pl_Other_Revenue678, pl_SocServ_Revenue678,
        nrows = 1, shareY = TRUE) %>% 
  layout(title = list(text = "Victorian Social Service Revenue Sources, 2016 to 2018",
                      standoff = 10,
                      xref = "canvas",
                      x = 0.1),
...
Other_Revenue678_long <- mutate(group_by(Other_Revenue678_long,
                                          main_activity, Year),
                                 revenue_proportion = revenue_amount / sum(revenue_amount,
                                                                           na.rm = TRUE),
                                 soc_serv_bool = if_else(str_detect(main_activity,
...
Revenue678_long <- rbind(Other_Revenue678_long,
                          SocServ_Revenue678_long)
...
## 2018 
FundingSources_18_long <- pivot_longer(select(VCOSS_ACNC_18,
                                              main_activity,
                                              charity_size,
...
SocServ_Total_FundingSources_18 <- filter(FundingSources_18_long,
                                          str_detect(main_activity, regex("social services",
                                                                          ignore_case = TRUE)))

SocServ_Total_FundingSources_18 <- summarise(group_by(SocServ_Total_FundingSources_18,
                                                      charity_size, main_activity, funding_source),
                                             funding_amount = sum(funding_amount))

SocServ_Total_FundingSources_18$Year <- 2018

## binding

SocServ_Total_FundingSources_678 <- rbind(SocServ_Total_FundingSources_16,
                                          SocServ_Total_FundingSources_17,
                                          SocServ_Total_FundingSources_18)

SocServ_Total_FundingSources_678 <- group_by(SocServ_Total_FundingSources_678,
                                              main_activity, charity_size, Year)

SocServ_Total_FundingSources_678 <- mutate(SocServ_Total_FundingSources_678,
                                            "funding_proportion" = (funding_amount / sum(funding_amount,
                                                                                         na.rm = TRUE)))

SocServ_Total_FundingSources_678_CharSize <- summarise(group_by(SocServ_Total_FundingSources_678,
                                                                 charity_size, funding_source, Year),
                                                        funding_amount = sum(funding_amount, na.rm = TRUE))

SocServ_Total_FundingSources_678_CharSize <- mutate(group_by(SocServ_Total_FundingSources_678_CharSize,
                                                              charity_size, Year),
                                                     funding_proportion = (funding_amount / sum(funding_amount,
                                                                                                na.rm = TRUE)))

gg_SocServ_FundingSources_CharSize_678 <- ggplot(SocServ_Total_FundingSources_678_CharSize) +
...
pl_SocServ_FundingSources_CharSize_678 <- ggplotly(gg_SocServ_FundingSources_CharSize_678,
                                                    tooltip = "text") %>% 
  layout(font = list(family = FSSIfont,
                     size = 12),
         yaxis = list(title = list(text = "Charity Size",
                                   standoff = 10)),
         title = list(text = "Social Service Funding Sources for each Charity Size",
                      xref = "canvas",
                      x = 0.1),
         margin = list(t = 55))
...
FundingSources_18_long <- filter(FundingSources_18_long,
                                 !str_detect(main_activity, regex("social services",
                                                                  ignore_case = TRUE)))

Total_FundingSources_18 <- summarise(group_by(FundingSources_18_long,
                                              charity_size, main_activity, funding_source),
                                     funding_amount = sum(funding_amount))

Total_FundingSources_18$Year <- 2018

## binding

Total_FundingSources_678 <- rbind(Total_FundingSources_16,
                                  Total_FundingSources_17,
                                  Total_FundingSources_18)

Total_FundingSources_678 <- group_by(Total_FundingSources_678,
                                      main_activity, charity_size, Year)

Total_FundingSources_678 <- mutate(Total_FundingSources_678,
                                    "funding_proportion" = (funding_amount / sum(funding_amount,
                                                                                 na.rm = TRUE)))

Total_FundingSources_678_CharSize <- summarise(group_by(Total_FundingSources_678,
                                                         charity_size, funding_source, Year),
                                                funding_amount = sum(funding_amount, na.rm = TRUE))

Total_FundingSources_678_CharSize <- mutate(group_by(Total_FundingSources_678_CharSize,
                                                      charity_size, Year),
                                             funding_proportion = (funding_amount / sum(funding_amount,
                                                                                        na.rm = TRUE)))

gg_FundingSources_CharSize_678 <- ggplot(Total_FundingSources_678_CharSize) +
  geom_bar(aes(x = funding_proportion,
               y = charity_size,
               fill = str_clean_wrap(funding_source),
               text = paste(sep = "\n",
...
pl_FundingSources_CharSize_678 <- ggplotly(gg_FundingSources_CharSize_678,
                                            tooltip = "text") %>% 
  layout(font = list(family = FSSIfont,
                     size = 12),
         yaxis = list(title = list(text = "Charity Size",
...
subplot(pl_FundingSources_CharSize_678,
        pl_SocServ_FundingSources_CharSize_678,
        nrows = 2) %>% 
  layout(title = list(text = "Funding Sources for each Charity Size",
         ...
         yaxis = list(title = list(text = "Charity Size\nOther Sectors")),
         yaxis2 = list(title = list(text = "Charity Size\nSocial Service")),
         xaxis4 = list(title = "Funding Proportion"),
         xaxis5 = list(title = "Funding Proportion"),
         xaxis6 = list(title = "Funding Proportion"))
...
pl_OtherRevenue <- plot_ly(data = filter(Revenue678_long,
                                         !str_detect(main_activity,
                                                     regex("social services", ignore_case = TRUE))),
                           x = ~revenue_amount,
  ...
  layout(barmode = "stack",
         ...
         title = list(text = "Total Funding Sources by Main Activity, 2016 to 2018",
                      xref = "canvas",
                      x = 0.1),
         font = list(family = FSSIfont,
                     size = 12))

pl_SocServ_Revene <- plot_ly(data = filter(Revenue678_long,
                                           str_detect(main_activity,
                                                      regex("social services", ignore_case = TRUE))),
                             x = ~revenue_amount,
                             y = ~str_clean_wrap(main_activity, 36),
...
gg_Other_Revenue_prop <- ggplot(filter(Revenue678_long,
                                    !str_detect(main_activity,
                                                regex("social services",
                                                      ignore_case = TRUE)))) +
...
gg_SocServ_Revenue_prop <- ggplot(filter(Revenue678_long,
                                      str_detect(main_activity,
                                                 regex("social services",
                                                       ignore_case = TRUE)))) +
  geom_bar(aes(x = revenue_proportion,
...
# Financial Health of Sector {.tabset .tabset-fade}

```{r echo=FALSE, message=FALSE, warning=FALSE}

# 2016

DeficitSurplus16 <- summarise(filter(VCOSS_ACNC_16,
                                     !str_detect(main_activity_2016,
                                                 regex("social services",
                                                       ignore_case = TRUE))),
                              deficit = sum(net_surplus_deficit_2016 < 0),
                              even = sum(net_surplus_deficit_2016 == 0),
                              surplus = sum(net_surplus_deficit_2016 > 0))

prop_charities_deficit16 <- sum(DeficitSurplus16$deficit) / sum(DeficitSurplus16$deficit,
                                                              DeficitSurplus16$even,
                                                              DeficitSurplus16$surplus)

SocServ_grantsout16 <- sum(filter(VCOSS_ACNC_16,
                                  str_detect(main_activity_2016,
                                             regex("Social Services",
                                                   ignore_case = TRUE)
                                  ))$grants_and_donations_made_for_use_outside_australia_2016)

SocServ_grantsin16 <- sum(filter(VCOSS_ACNC_16,
                                 str_detect(main_activity_2016,
                                            regex("Social Services",
                                                  ignore_case = TRUE)
                                 ))$grants_and_donations_made_for_use_in_australia_2016)


# 2017

DeficitSurplus17 <- summarise(filter(VCOSS_ACNC_17,
                                     !str_detect(main_activity,
                                                 regex("social services",
                                                       ignore_case = TRUE))),
                              deficit = sum(net_surplus_deficit_2017 < 0),
                              even = sum(net_surplus_deficit_2017 == 0),
                              surplus = sum(net_surplus_deficit_2017 > 0))

prop_charities_deficit17 <- sum(DeficitSurplus17$deficit) / sum(DeficitSurplus17$deficit,
                                                              DeficitSurplus17$even,
                                                              DeficitSurplus17$surplus)

SocServ_grantsin17 <- sum(filter(VCOSS_ACNC_17,
                                 str_detect(main_activity,
                                            regex("Social Services",
                                                  ignore_case = TRUE)
                                 ))$grants_and_donations_made_for_use_in_australia_2017)

SocServ_grantsout17 <- sum(filter(VCOSS_ACNC_17,
                                  str_detect(main_activity,
                                             regex("Social Services",
                                                   ignore_case = TRUE)
                                  ))$grants_and_donations_made_for_use_outside_australia_2017)

# 2018

DeficitSurplus18 <- summarise(filter(VCOSS_ACNC_18,
                                     !str_detect(main_activity,
                                                 regex("social services",
                                                       ignore_case = TRUE))),
                              deficit = sum(net_surplus_deficit < 0),
                              even = sum(net_surplus_deficit == 0),
                              surplus = sum(net_surplus_deficit > 0))

prop_charities_deficit18 <- sum(DeficitSurplus18$deficit) / sum(DeficitSurplus18$deficit,
                                                              DeficitSurplus18$even,
                                                              DeficitSurplus18$surplus)

SocServ_grantsin18 <- sum(filter(VCOSS_ACNC_18,
                                 str_detect(main_activity,
                                            regex("Social Services",
                                                  ignore_case = TRUE)
                                 ))$grants_and_donations_made_for_use_in_australia)

SocServ_grantsout18 <- sum(filter(VCOSS_ACNC_18,
                                  str_detect(main_activity,
                                             regex("Social Services",
                                                   ignore_case = TRUE)
...
The Social Service sector had the second-highest percentage of charities and not-for-profit organisations running a budget deficit in 2016 (38.2%) as well as in 2017 (39.4%). This is compared to `r percent(prop_charities_deficit16, 0.1)` of all other charities running a budget deficit in 2016 and `r percent(prop_charities_deficit17, 0.1)` of all other charities in deficit in 2017.  
Compared to other sectors such as *International Activities* and *Other Education*, charities performing Social Service receive a relatively low amount of funding for grants and donations to be used in Australia or outside of Australia. Noticeably, the amount of grants and donations for use outside of Australia by Social Service charities increased from `r dollar(SocServ_grantsout16)` in 2016 to `r dollar(SocServ_grantsout17)` in 2017; a `r round(SocServ_grantsout17 / SocServ_grantsout16, 1)` fold increase between years.
...
SocServ_DebtSurplus_MainAct_18 <- summarise(group_by(filter(VCOSS_ACNC_18,
                                                            str_detect(main_activity,
                                                                       regex("social services", 
                                                                             ignore_case = TRUE))),
                                                     main_activity),
...
SocServ_DebtSurplus_MainAct_18$Year <- 2018

# binding

SocServ_DebtSurplus_MainAct_678 <- rbind(SocServ_DebtSurplus_MainAct_16,
                                         SocServ_DebtSurplus_MainAct_17,
                                         SocServ_DebtSurplus_MainAct_18)

SocServ_DebtSurplus_MainAct_678_long <- pivot_longer(SocServ_DebtSurplus_MainAct_678,
                                                     cols = c(net_debt,
                                                              net_surplus,
                                                              net_balanced),
...
SocServ_DebtSurplus_MainAct_678_long$Year <- as.ordered(SocServ_DebtSurplus_MainAct_678_long$Year)

gg_SocServ_DebtSurplus <- ggplot(SocServ_DebtSurplus_MainAct_678_long,
                                 aes(x = Year,
                                     y = count,
...
ggplotly(gg_SocServ_DebtSurplus,
         tooltip = "text") %>% 
  layout(title = list(text = "Count of Social Service Charities by Budget Status, 2016 - 2018",
                      standoff = 10,
                      xref = "canvas",
                      x = 0.1),
         font = list(family = FSSIfont,
...
### Social Service Budget Status per Charity Size

```{r echo=FALSE, message=FALSE, warning=FALSE}

# 2016

DebtSurplus_MainAct_CharSize_16 <- summarise(group_by(VCOSS_ACNC_16,
                                                      main_activity = main_activity_2016,
                                                      charity_size),
                                             deficit = sum(net_surplus_deficit_2016 < 0),
                                             balanced = sum(net_surplus_deficit_2016 == 0),
                                             surplus = sum(net_surplus_deficit_2016 > 0))

DebtSurplus_MainAct_CharSize_16$Year <- 2016

# 2017

DebtSurplus_MainAct_CharSize_17 <- summarise(group_by(VCOSS_ACNC_17,
                                                      main_activity, charity_size),
                                             deficit = sum(net_surplus_deficit_2017 < 0),
                                             balanced = sum(net_surplus_deficit_2017 == 0),
                                             surplus = sum(net_surplus_deficit_2017 > 0))

DebtSurplus_MainAct_CharSize_17$Year <- 2017

# 2018

DebtSurplus_MainAct_CharSize_18 <- summarise(group_by(VCOSS_ACNC_18,
                                                      main_activity, charity_size),
                                             deficit = sum(net_surplus_deficit < 0),
                                             balanced = sum(net_surplus_deficit == 0),
                                             surplus = sum(net_surplus_deficit > 0))

DebtSurplus_MainAct_CharSize_18$Year <- 2018

# binding

DebtSurplus_MainAct_CharSize_678 <- rbind(DebtSurplus_MainAct_CharSize_16,
                                           DebtSurplus_MainAct_CharSize_17,
                                           DebtSurplus_MainAct_CharSize_18)

DebtSurplus_MainAct_CharSize_678_long <- pivot_longer(DebtSurplus_MainAct_CharSize_678,
                                                       cols = c(deficit,
                                                                balanced,
                                                                surplus),
                                                       names_to = "budget_status",
...
DebtSurplus_MainAct_CharSize_678_long <- mutate(group_by(DebtSurplus_MainAct_CharSize_678_long,
                                                          main_activity, charity_size, Year),
                                                 proportion = (count / sum(count)))

SocServ_DebtSurplus_CharSize_678 <- filter(DebtSurplus_MainAct_CharSize_678_long,
                                            str_detect(main_activity,
                                                       regex("social services",
                                                             ignore_case = TRUE)))
...
gg_SocServ_DebtSurplus_CharSize_678 <- ggplot(SocServ_DebtSurplus_CharSize_678) +
  geom_bar(aes(x = count,
               y = charity_size,
               fill = budget_status,
               text = paste(sep = "\n",
                            paste(comma(count, 1), charity_size, "charities"),
                            paste("Operating a", budget_status, "budget in"),
                            Year,
                            paste0("(", percent(proportion, 0.1), " of ", charity_size, " charities)"))),
           stat = "identity") +
  facet_wrap(~Year) +
  scale_x_continuous("",
                     labels = comma) +
...
pl_SocServ_DebtSurplus_CharSize_678 <- ggplotly(gg_SocServ_DebtSurplus_CharSize_678,
                                                 tooltip = "text")

gg_SocServ_DebtSurplus_CharSize_678_prop <- ggplot(SocServ_DebtSurplus_CharSize_678) +
  geom_bar(aes(x = proportion,
               y = charity_size,
               fill = budget_status,
               text = paste(sep = "\n",
                            paste(percent(proportion, 0.1), "of", charity_size, "charities"),
                            paste("Operating a", budget_status, "budget in"),
                            Year,
                            paste0("(", comma(count, 1), " charities)"))),
           stat = "identity") +
  facet_wrap(~Year) +
  scale_x_continuous("",
                     labels = percent) +
...
pl_SocServ_DebtSurplus_CharSize_678_prop <- ggplotly(gg_SocServ_DebtSurplus_CharSize_678_prop,
                                                      tooltip = "text")

subplot(pl_SocServ_DebtSurplus_CharSize_678,
        pl_SocServ_DebtSurplus_CharSize_678_prop,
        nrows = 2,
        margin = 0.066) %>% 
  layout(title = list(text = "Social Service Budget status by Charity Size, 2016 to 2018",
                      standoff = 10,
                      xref = "canvas",
                      x = 0.1),
         legend = list(title = list(text = "Budget status",
         ...
         font = list(family = FSSIfont,
                     size = 12),
         xaxis = list(title = list(text = "Number of Charities",
                                   standoff = 5)),
         xaxis2 = list(title = list(text = "Number of Charities",
                                   standoff = 5)),
         xaxis3 = list(title = list(text = "Number of Charities",
                                   standoff = 5)),
...
ggplotly(gg_Grants_MainAct_678,
                 tooltip = "text") %>% 
  layout(showlegend = TRUE,
         title = list(text = "Grants and Donations for Use In and Outside Australia by Main Activity, 2016 to 2018",
                      xref = "canvas",
                      x = 0.1),
...
ggplotly(gg_Grants_InOut_MainAct_678,
         tooltip = "text") %>% 
  layout(title = list(text = "Grant Use for Social Service, 2016 to 2018",
                      standoff = 10,
                      xref = "canvas",
                      x = 0.1),
         font = list(family = FSSIfont,
...
### 1c - Cleaned Datasets {.tabset .tabset-fade}
...
VIC_Others_678 <- rbind(VIC_Others16, VIC_Others17)

VIC_Others_678$Year <- as.ordered(VIC_Others_678$Year)

Sum_VIC_Others_678 <- summarise(group_by(VIC_Others_678,
                                           Year, charity_size, main_activity),
                                  n_Charities = n())

gg_Sum_VIC_Others_678 <- ggplot(Sum_VIC_Others_678,
                                  aes(x = Year, y = n_Charities,
                                      col = fct_rev(charity_size),
                                      group = fct_rev(charity_size),
                                      text = paste(sep = "\n",
...
ggplotly(gg_Sum_VIC_Others_678,
         tooltip = "text", shareY = FALSE) %>% 
  layout(title = list(text = "Number of Charities in Other Sectors, 2016 to 2018",
                      standoff = 10,
                      xref = "canvas",
                      x = 0.1),
         font = list(family = FSSIfont,
...
Other_Staff_CharSize_678 <- rbind(Other_Staff_CharSize_16, Other_Staff_CharSize_17)

Other_Staff_CharSize_678 <- mutate(group_by(Other_Staff_CharSize_678,
                                             Year, charity_size),
                                    proportion = (employee_count / sum(employee_count, na.rm = TRUE)))

gg_Other_Staff_CharSize_678 <- ggplot(Other_Staff_CharSize_678) +
  geom_bar(aes(x = employee_count,
               y = charity_size,
               fill = str_clean_wrap(employment_type),
               text = paste(sep = "\n",
...
pl_Other_Staff_CharSize_678 <- ggplotly(gg_Other_Staff_CharSize_678,
                                   tooltip = "text") %>% 
  layout(yaxis = list(title = list(text = "Charity Size",
                                   standoff = 10)),
         font = list(family = FSSIfont,
...
gg_Other_Staff_CharSize_678_prop <- ggplot(Other_Staff_CharSize_678) +
  geom_bar(aes(x = proportion,
               y = charity_size,
               fill = str_clean_wrap(employment_type),
               text = paste(sep = "\n",
...
pl_Other_Staff_CharSize_678_prop <- ggplotly(gg_Other_Staff_CharSize_678_prop,
                                        tooltip = "text",
                                        showlegend = FALSE) %>% 
  layout(yaxis = list(title = list(text = "Charity Size",
                                   standoff = 10)),
...
subplot(pl_Other_Staff_CharSize_678, pl_Other_Staff_CharSize_678_prop,
        nrows = 2, margin = 0.1) %>% 
...
Other_Staff_MainAct_678 <- rbind(Other_Staff_MainAct16_long, Other_Staff_MainAct17_long)

Other_Staff_MainAct_678$Year <- as.ordered(Other_Staff_MainAct_678$Year)

gg_Other_Staff_MainAct_678 <- ggplot(Other_Staff_MainAct_678,
                                      aes(x = Year,
                                          y = employee_count,
                                          col = str_clean_wrap(employment_type),
                                          group = str_clean_wrap(employment_type),
...
ggplotly(gg_Other_Staff_MainAct_678,
         tooltip = "text") %>% 
  layout(title = list(text = "Changes in Staff Counts by Work Loading in Other Sectors, 2016 to 2018",
                      standoff = 10,
                      xref = "canvas",
                      x = 0.1),
         font = list(family = FSSIfont,
...
gg_Other_Volunteers_PaidStaff_678 <- ggplot(filter(Volunteers_PaidStaff_678,
                                                    !str_detect(main_activity,
                                                                regex("social services",
                                                                      ignore_case = TRUE)))) +
  geom_abline(slope = 1,
...
ggplotly(gg_Other_Volunteers_PaidStaff_678,
         tooltip = "text") %>% 
  add_annotations(text = "equal number volunteers to paid staff",
                  textangle = -33.42,
                  opacity = 0.5,
  ...
  layout(legend = list(title = list(text = "Charity size and Main Activity",
                       ...
                       itemclick = "toggleothers",
                       itemdoubleclick = "toggle"),
         title = list(text = "Volunteers and Paid Staff Working for Other Sectors, 2016 to 2018",
                      xref = "canvas",
                      x = 0.1),
         margin = list(t = 50,
                       b = 10,
...
plot_ly() %>% 
  add_pie(data = filter(Volunteers_PaidStaff_678_long,
  ...
  layout(grid = list(rows = 4, columns = 3),
         title = list(text = "Volunteer Workforce Percentage for Other Sectors, 2016 to 2018",
                      standoff = 5,
                      xref = "canvas",
                      x = 0.1),
         font = list(family = FSSIfont,
...
gg_Other_DebtSurplus <- ggplot(Other_DebtSurplus_MainAct_678_long,
                               aes(x = Year,
                                   y = count,
                                   colour = str_cleaner(budget_status),
                                   group = budget_status,
...
ggplotly(gg_Other_DebtSurplus,
         tooltip = "text") %>% 
  layout(title = list(text = "Count of Other Sector Charities by Budget Status, 2016 - 2018",
                      standoff = 10,
                      xref = "canvas",
                      x = 0.1),
         font = list(family = FSSIfont,
...
Grants_InOut_MainAct_678 <- rbind(Grants_InOut_MainAct_16, Grants_InOut_MainAct_17)
...
ggplotly(gg_Grants_InOut_MainAct_678,
  ...
  layout(title = list(text = "Grant Use for Other Sectors, 2016 to 2018",
                      standoff = 10,
                      xref = "canvas",
                      x = 0.1),
         font = list(family = FSSIfont,
